...
import { Order, OrderState } from 'store/orderSlice'
...
const Chart = () => {
  ...
  const backgroundColors = ['rgba(255, 99, 132, 0.5)', 'rgba(53, 162, 235, 0.5)', 'rgba(75, 192, 192, 1)', 'rgba(153, 102, 255, 1)',]
  const stateFilteredOrders = useCustomSelector(state => state.orders.defaultChartData.orderStateChart);

  const data = {
    labels: timeRangeValues.map(v => v.label),
    datasets: (Object.keys(stateFilteredOrders) as OrderState[]).map((type, i) => ({
      fill: true,
      label: type,
      data: stateFilteredOrders[type],
